'use strict';
const { applyHomeyUserCosts } = require('./price-formula');
...
function localMidnightUtcMs(dateKey, timezone = 'UTC') {
  const match = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(dateKey || ''));
  if (!match) return null;
  const targetWallMs = Date.UTC(Number(match[1]), Number(match[2]) - 1, Number(match[3]), 0, 0, 0, 0);
  let guess = targetWallMs;
  // Resolve local midnight to UTC without relying on the host timezone. The
  // iterative correction also handles daylight-saving offsets.
  for (let i = 0; i < 4; i += 1) {
    const lp = localParts(new Date(guess), timezone);
    const observedWallMs = Date.UTC(lp.year, lp.month - 1, lp.day, lp.hour, lp.minute, 0, 0);
    const delta = targetWallMs - observedWallMs;
    guess += delta;
    if (delta === 0) break;
  }
  return guess;
...
function localDateTimeUtcMs(dateKey, timeValue = '00:00', timezone = 'UTC') {
  ...
  const targetWallMs = Date.UTC(
    Number(dateMatch[1]), Number(dateMatch[2]) - 1, Number(dateMatch[3]), hour, minute, 0, 0,
  );
  let guess = targetWallMs;
  // Resolve arbitrary local wall-clock times to UTC. The iterative correction
  // keeps boiler day boundaries stable across DST changes and does not depend
  // on the host operating-system timezone.
  for (let i = 0; i < 4; i += 1) {
    const lp = localParts(new Date(guess), timezone);
    const observedWallMs = Date.UTC(lp.year, lp.month - 1, lp.day, lp.hour, lp.minute, 0, 0);
    const delta = targetWallMs - observedWallMs;
...
function sequentialPricePeriodInfo(period = 'this_day', intervalMinutes = 15, now = new Date(), timezone = 'UTC') {
  const interval = Math.max(1, Math.round(Number(intervalMinutes) || 15));
  const stepMs = interval * 60 * 1000;
  const lp = localParts(now, timezone);
  let dateKey = lp.dateKey;
  if (period === 'tomorrow') dateKey = shiftDateKey(dateKey, 1);

  if (period === 'this_day' || period === 'tomorrow') {
    const startMs = localMidnightUtcMs(dateKey, timezone);
    const endMs = localMidnightUtcMs(shiftDateKey(dateKey, 1), timezone);
    if (!Number.isFinite(startMs) || !Number.isFinite(endMs) || endMs <= startMs) return null;
    return {
      period,
      intervalMinutes: interval,
      startMs,
      expectedCount: Math.round((endMs - startMs) / stepMs),
      dateKey,
    };
  }

  if (period === 'next_hours') {
    const dayStartMs = localMidnightUtcMs(lp.dateKey, timezone);
    if (!Number.isFinite(dayStartMs)) return null;
    const elapsedMs = Math.max(0, now.getTime() - dayStartMs);
    const startMs = dayStartMs + (Math.floor(elapsedMs / stepMs) * stepMs);
    return {
      period,
      intervalMinutes: interval,
      startMs,
...
function normalizeSequentialPriceArray(payload, { period = 'this_day', intervalMinutes = 15, now = new Date(), timezone = 'UTC' } = {}) {
  if (!Array.isArray(payload) || payload.length === 0) return [];
  const numeric = payload.map(value => Number(value));
  if (numeric.some(value => !Number.isFinite(value))) return [];
  const info = sequentialPricePeriodInfo(period, intervalMinutes, now, timezone);
  if (!info) return [];
  const normalized = normalizePriceUnit(numeric.map((price, index) => ({ price, index })));
  const stepMs = info.intervalMinutes * 60 * 1000;
  return normalized.map((row, index) => {
    const startMs = info.startMs + (index * stepMs);
    const lp = localParts(new Date(startMs), timezone);
    return {
      startMs,
      dateKey: lp.dateKey,
      minute: lp.minuteOfDay,
      price: row.price,
      rawPrice: row.price,
      userCostAdder: 0,
      raw: payload[index],
...
function normalizeDynamicPriceResponse(payload, { dateHint = '', timezone = 'UTC', userCosts = null } = {}) {
  const rows = [];
  ...
  const adjusted = applyHomeyUserCosts(payload, normalized, userCosts);
