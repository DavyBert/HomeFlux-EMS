...
const DEFAULTS = {
  ...
  individualBatteryPowerLimitsEnabled: false,
  battery1CapacityKwh: null,
  battery1MaxChargeW: 2300, battery1MaxDischargeW: 2400,
  battery2CapacityKwh: null,
  battery2MaxChargeW: 2300, battery2MaxDischargeW: 2400,
  battery3CapacityKwh: null,
  battery3MaxChargeW: 2300, battery3MaxDischargeW: 2400,
  battery4CapacityKwh: null,
  battery4MaxChargeW: 2300, battery4MaxDischargeW: 2400,
  battery5CapacityKwh: null,
  battery5MaxChargeW: 2300, battery5MaxDischargeW: 2400,
  battery6CapacityKwh: null,
  battery6MaxChargeW: 2300, battery6MaxDischargeW: 2400,
  battery7CapacityKwh: null,
  battery7MaxChargeW: 2300, battery7MaxDischargeW: 2400,
  battery8CapacityKwh: null,
  battery8MaxChargeW: 2300, battery8MaxDischargeW: 2400,
  balanceEnabled: true,
  ...
  dynamicNormalChargeEnabled: false,
  dynamicNormalChargeMaxSoc: 50,
  ...
  // Optional v0.4.0 escape hatch: a forecast-classified low-PV day can be
  // promoted to a normal sunny day after the average battery SoC proves that
  // holding stored energy is no longer useful. The app owns the timer/latch.
  lowForecastAutoSunnyEnabled: false,
  ...
  // Separate planning deadlines: the night plan only bridges to the configured
  // morning target; the day plan only bridges from that morning target to the
  // configured daytime target. This prevents an evening night plan from
  // planning all the way through the following afternoon.
  nightTargetTime: '07:00',
  solarTargetTime: '17:00',
  touRates: [
    { id: 'superdal', name: 'Superdal', importPrice: 0, feedInPrice: 0, weekdayChargeMode: 'night', weekendChargeMode: 'night', nightChargeAllowed: true, dayChargeAllowed: false, avoidGridImport: false, lowForecastDischargeToTarget: false, evChargeAllowed: true, evPvChargeAllowed: false, evPvGridTopUpAllowed: false, evPvMinSurplusW: 0, evPvStopGridImportW: 1000, evMaxGridImportW: 0 },
    { id: 'dal', name: 'Dal', importPrice: 0, feedInPrice: 0, weekdayChargeMode: 'day', weekendChargeMode: 'day', nightChargeAllowed: false, dayChargeAllowed: true, avoidGridImport: false, lowForecastDischargeToTarget: false, evChargeAllowed: false, evPvChargeAllowed: true, evPvGridTopUpAllowed: false, evPvMinSurplusW: 0, evPvStopGridImportW: 1000, evMaxGridImportW: 0 },
    { id: 'piek', name: 'Piek', importPrice: 0, feedInPrice: 0, weekdayChargeMode: 'never', weekendChargeMode: 'never', nightChargeAllowed: false, dayChargeAllowed: false, avoidGridImport: true, lowForecastDischargeToTarget: false, evChargeAllowed: false, evPvChargeAllowed: true, evPvGridTopUpAllowed: false, evPvMinSurplusW: 0, evPvStopGridImportW: 1000, evMaxGridImportW: 0 },
  ],
  touSchedule: [
  ...
  // Legacy setting key retained for upgrades; value is now the number of last
  // received grid inputs to average (0 = direct, supported: 3/5/7/10).
  ...
  evCount: 0,
  // Settings-only advisory input used to preview EV charging headroom against
  // Peak Guard before a session starts. Live control always uses the real P1.
  evIdleHouseLoadW: 0,
  evName: '',
  evEnabled: false,
  evSocEnabled: true,
  evSocFreshnessMinutes: 15,
  evMode: 'smart',
  evEmergencyDynamicCheapEnabled: true,
  evEmergencyDynamicNormalEnabled: true,
  evEmergencyDynamicExpensiveEnabled: true,
  evEmergencyFixedWindowEnabled: true,
  evEmergencyFixedOutsideWindowEnabled: true,
  evControlType: 'current', // current = ampere setpoint, mode = stop/smart/standard, hybrid = both
  evSmartPvPriority: 'battery_first', // legacy compatibility; global evPvSharePercent is authoritative
  evPvSharePercent: 10,
  ...
  evPvGridTopUpMode: 'full', // off | minimum | full; full preserves pre-0.6.1 behaviour
  evPvStartSurplusW: 1500,
  evPvStopSurplusW: 800,
  evPvStopDelaySeconds: 60,
  evPhases: 3,
  // Legacy snapshot fallback; migration initializes the phase-specific supply voltage.
  evVoltage: 230,
  evVoltageReference: 'phase',
  evMinCurrentA: 6,
  evMaxCurrentA: 32,
  ...
  evFeedbackTolerancePercent: 15,
  evSkipFeedbackValidation: false,
  evPeakGuardStopHoldSeconds: 60,
  evPeakGuardBatteryAssistNormal: false,
  // Optional night-planning source: EV may consume only home-battery energy
  // above the calculated night target SoC. A per-session start gate in app.js
  // requires >1.0 kWh transferable before the night target time.
  evNightSurplusChargeEnabled: false,
  evPeakGuardBatteryAssistEmergency: false,
  evFixedChargeWindowEnabled: true,
  evDynamicCheapEnabled: true,
  evDynamicNormalEnabled: false,
  evDynamicExpensiveEnabled: false,
  evFixedMaxGridImportW: 0,
  evDynamicCheapMaxGridImportW: 0,
  evDynamicNormalMaxGridImportW: 0,
  evDynamicExpensiveMaxGridImportW: 0,
  evSocDeadlineOverrides: [],
...
const EV_INSTANCE_DEFAULT_KEYS = [
  'Enabled','Name','SocEnabled','SocFreshnessMinutes','Mode','EmergencyDynamicCheapEnabled','EmergencyDynamicNormalEnabled','EmergencyDynamicExpensiveEnabled','EmergencyFixedWindowEnabled','EmergencyFixedOutsideWindowEnabled','ControlType','SmartPvPriority','SmartPvExportTargetW',
  'SmartGridPriority','Weight','BatteryCapacityKwh','TargetSoc','TargetTime','GuaranteeTarget','AllowUnselectedTariffForDeadline',
  'PvGridTopUpMode','PvStartSurplusW','PvStopSurplusW','PvStopDelaySeconds','Phases','Voltage','VoltageReference',
  'MinCurrentA','MaxCurrentA','StandardCurrentA','ModeSmartCurrentA','ModeStandardCurrentA','CommandIntervalSeconds','FeedbackTolerancePercent','SkipFeedbackValidation','PeakGuardStopHoldSeconds',
  'PeakGuardBatteryAssistNormal','NightSurplusChargeEnabled','PeakGuardBatteryAssistEmergency','FixedChargeWindowEnabled','DynamicCheapEnabled','DynamicNormalEnabled','DynamicExpensiveEnabled',
];
const EV_INSTANCE_DEFAULT_SOURCE = {
  Enabled: DEFAULTS.evEnabled,
  Name: '',
  SocEnabled: DEFAULTS.evSocEnabled,
  SocFreshnessMinutes: DEFAULTS.evSocFreshnessMinutes,
  Mode: DEFAULTS.evMode,
  EmergencyDynamicCheapEnabled: DEFAULTS.evEmergencyDynamicCheapEnabled,
  EmergencyDynamicNormalEnabled: DEFAULTS.evEmergencyDynamicNormalEnabled,
  EmergencyDynamicExpensiveEnabled: DEFAULTS.evEmergencyDynamicExpensiveEnabled,
  EmergencyFixedWindowEnabled: DEFAULTS.evEmergencyFixedWindowEnabled,
  EmergencyFixedOutsideWindowEnabled: DEFAULTS.evEmergencyFixedOutsideWindowEnabled,
  ...
  AllowUnselectedTariffForDeadline: DEFAULTS.evAllowUnselectedTariffForDeadline,
  PvGridTopUpMode: DEFAULTS.evPvGridTopUpMode,
  PvStartSurplusW: DEFAULTS.evPvStartSurplusW,
  PvStopSurplusW: DEFAULTS.evPvStopSurplusW,
  PvStopDelaySeconds: DEFAULTS.evPvStopDelaySeconds,
  Phases: DEFAULTS.evPhases,
  Voltage: DEFAULTS.evVoltage,
  VoltageReference: DEFAULTS.evVoltageReference,
  MinCurrentA: DEFAULTS.evMinCurrentA,
  MaxCurrentA: DEFAULTS.evMaxCurrentA,
  StandardCurrentA: DEFAULTS.evStandardCurrentA,
  ModeSmartCurrentA: DEFAULTS.evModeSmartCurrentA,
  ModeStandardCurrentA: DEFAULTS.evModeStandardCurrentA,
  CommandIntervalSeconds: DEFAULTS.evCommandIntervalSeconds,
  FeedbackTolerancePercent: DEFAULTS.evFeedbackTolerancePercent,
  SkipFeedbackValidation: DEFAULTS.evSkipFeedbackValidation,
  PeakGuardStopHoldSeconds: DEFAULTS.evPeakGuardStopHoldSeconds,
  PeakGuardBatteryAssistNormal: DEFAULTS.evPeakGuardBatteryAssistNormal,
  NightSurplusChargeEnabled: DEFAULTS.evNightSurplusChargeEnabled,
  PeakGuardBatteryAssistEmergency: DEFAULTS.evPeakGuardBatteryAssistEmergency,
...
for (const rate of DEFAULTS.touRates) {
  ...
  rate.evPvGridTopUpAllowed = Boolean(rate.evChargeAllowed);
  for (let instance = 2; instance <= 4; instance += 1) {
    ...
    rate[`ev${instance}PvGridTopUpAllowed`] = false;
  }
}
...
function normalizedBatteryCount(settings = {}) {
  ...
  return Math.round(clamp(Number.isFinite(configured) ? configured : fallback, 0, 8));
}
...
function resolvePlanningPhaseBounds(inputState = {}, settings = {}, nowInput = new Date()) {
  ...
  const dayTargetMinute = parseTime(settings.solarTargetTime);
  const nightTargetMinute = parseTime(settings.nightTargetTime);
  const dayMinute = dayTargetMinute === null ? 17 * 60 : dayTargetMinute;
  const nightMinute = nightTargetMinute === null ? 7 * 60 : nightTargetMinute;

  // The two configured target times deliberately split the planning day:
  // night: previous daytime target -> morning target
  // day:   morning target -> daytime target
  // Availability calculations still use max(now, block start), so elapsed
  // parts of a phase are display context only and can never be charged again.
  const startAt = nightPlanning
    ? localDateAtMinute(previousDate, dayMinute, timezone).getTime()
    : localDateAtMinute(targetDate, nightMinute, timezone).getTime();
  const targetAt = localDateAtMinute(targetDate, nightPlanning ? nightMinute : dayMinute, timezone).getTime();

  return {
    nightPlanning,
    phase: nightPlanning ? 'night' : 'day',
    targetDate,
    previousDate,
    targetDateKey: targetDate.dateKey,
    startAt,
    targetAt,
    targetLabel: nightPlanning ? 'Nacht-doelmoment' : 'Dag-doelmoment',
  };
}
...
function resolvePlanningCycle(inputState = {}, settings = {}, nowInput = new Date()) {
  const now = nowInput instanceof Date ? nowInput : new Date(nowInput);
  const timezone = settings.timezone || 'UTC';
  const phase = resolvePlanningPhaseBounds(inputState, settings, now);
  const { targetDate, startAt: phaseStartAt, targetAt, nightPlanning } = phase;
  ...
  if (settings.contractType === 'tou') {
    ...
    const cheapBlocks = mergePlanningBlocks(occurrences
      .filter(item => (nightPlanning ? item.nightChargeAllowed : item.dayChargeAllowed)
        && item.endAt > phaseStartAt && item.startAt < targetAt)
      .map(item => ({
        startAt: Math.max(item.startAt, phaseStartAt),
        endAt: Math.min(item.endAt, targetAt),
    ...
    return {
      ...
      targetDateKey: phase.targetDateKey,
      startAt: phaseStartAt,
      chargeDeadlineAt: targetAt,
      peakStartAt: targetAt,
      peakEndAt: targetAt,
      peakLabel: phase.targetLabel,
      endAt: targetAt,
      cheapBlocks,
    };
  }
  ...
  if (!isDynamicContract(settings)) {
    const cheapBlocks = mergePlanningBlocks(buildFixedPlanningOccurrences(settings, targetDate, timezone)
      .filter(item => item.endAt > phaseStartAt && item.startAt < targetAt)
      .map(item => ({
        startAt: Math.max(item.startAt, phaseStartAt),
        endAt: Math.min(item.endAt, targetAt),
        tariffLabel: item.tariffLabel,
...
function findCurrentTariff(now, settings, planningState = null) {
  ...
  if (isDynamicContract(settings)) {
    ...
    if (settings.dynamicNormalChargeEnabled && className === 'normal') {
      const bounds = resolvePlanningPhaseBounds(planningState || {}, settings, now);
      selectedChargeMinutes = slots.reduce((sum, slot) => {
        if (cheap.has(slot.minute) || expensive.has(slot.minute)) return sum;
        const startAt = localDateAtMinute(zonedParts(now, timezone), slot.minute, timezone).getTime();
        return sum + Math.max(0, Math.min(startAt + intervalMinutes * 60000, bounds.targetAt)
          - Math.max(startAt, now.getTime())) / 60000;
      }, 0);
    }
...
function validBatterySocValues(state, settings) {
  const count = normalizedBatteryCount(settings);
...
function validBatterySocIndices(state, settings) {
  const count = normalizedBatteryCount(settings);
  const values = Array.isArray(state.batterySoc) ? state.batterySoc : [];
  const indices = [];
  for (let index = 0; index < count; index += 1) {
    const value = values[index];
    if (value !== null && value !== undefined && value !== '' && Number.isFinite(Number(value))) indices.push(index);
  }
  return indices;
}

function batteryPowerLimits(settings, index) {
  const globalMaxCharge = Math.max(0, asNumber(settings.maxChargePerBatteryW, 2300));
  const globalMaxDischarge = Math.max(0, asNumber(settings.maxDischargePerBatteryW, 2400));
  if (!Boolean(settings.individualBatteryPowerLimitsEnabled)) {
    return { minCharge: 0, maxCharge: globalMaxCharge, minDischarge: 0, maxDischarge: globalMaxDischarge };
  }
  const battery = Math.max(1, Math.min(8, Number(index) + 1));
  const maxCharge = Math.max(0, asNumber(settings[`battery${battery}MaxChargeW`], globalMaxCharge));
  const maxDischarge = Math.max(0, asNumber(settings[`battery${battery}MaxDischargeW`], globalMaxDischarge));
  // General heterogeneous-battery settings define maxima only. Hardware-specific
  // minimum power belongs to Split Command and is applied at publication time.
  return { minCharge: 0, maxCharge, minDischarge: 0, maxDischarge };
}

function aggregateBatteryPowerLimits(settings, indices = null) {
  const count = normalizedBatteryCount(settings);
  const selected = Array.isArray(indices) ? indices : Array.from({ length: count }, (_, index) => index);
  let maxCharge = 0;
  let maxDischarge = 0;
  for (const index of selected) {
    if (index < 0 || index >= count) continue;
    const limits = batteryPowerLimits(settings, index);
    maxCharge += limits.maxCharge;
    maxDischarge += limits.maxDischarge;
  }
  return { maxCharge, maxDischarge };
}

function batteryCapacityKwh(settings, index) {
  const count = normalizedBatteryCount(settings);
  const shared = count > 0 ? Math.max(0, asNumber(settings.totalCapacityKwh, 21.6)) / count : 0;
  const individual = Number(settings[`battery${index + 1}CapacityKwh`]);
  return settings.individualBatteryPowerLimitsEnabled && Number.isFinite(individual) && individual > 0
    ? individual : shared;
}

function configuredBatteryCapacityKwh(settings) {
  return Array.from({ length: normalizedBatteryCount(settings) }, (_, index) => batteryCapacityKwh(settings, index))
...
function computeAverageSoc(state, settings) {
  ...
  if (!settings.individualBatteryPowerLimitsEnabled) return valid.reduce((sum, value) => sum + value, 0) / valid.length;
  const indices = validBatterySocIndices(state, settings);
  const capacity = indices.reduce((sum, index) => sum + batteryCapacityKwh(settings, index), 0);
  return capacity > 0
    ? indices.reduce((sum, index) => sum + Number(state.batterySoc[index]) * batteryCapacityKwh(settings, index), 0) / capacity
    : valid.reduce((sum, value) => sum + value, 0) / valid.length;
}

function computeEffectiveCapacityKwh(state, settings) {
  return validBatterySocIndices(state, settings).reduce((sum, index) => sum + batteryCapacityKwh(settings, index), 0);
}
...
function computePeakReserveDetails(state, settings, forecastContext = null, now = new Date()) {
  ...
  // v0.6.2: the applicable monthly minimum is an absolute planning floor.
  // Selected months use the selected-month minimum only when that calendar
  // month is enabled. Non-selected months use the separate sunny-month minimum
  // only when sunnyMonthsMinSocEnabled is enabled. During day planning the
  // applicable minimum may no longer be reduced by a PV forecast: forecast PV
  // is expected energy, not SoC that is already present. During night planning
  // the same floor is applied only when peakReserveNightEnabled is enabled.
  // Keep the PV-credit diagnostics for visibility, but they no longer lower
  // the minimum SoC target itself.
  const pvAvailableForReserveKwh = active && forecastKwh !== null
    ? (nightReservePhase ? 0 : forecastKwh)
    : 0;
  ...
  const daytimeReserveTargetSoc = active
    ? strategyConfiguredTargetSoc
    : safetySoc;

  // v0.7.13: monthly minimums are floors, never an extra energy requirement.
  // The ordinary forecast target is calculated separately from the technical
  // Minimum SoC. Day and night planning then apply the applicable seasonal
  // minimum only as a lower bound, so it is not added on top of forecast need.
  const nightCombinedGapKwh = 0;
  const nightReserveTargetSoc = active && nightReservePhase
    ? strategyConfiguredTargetSoc
    : safetySoc;
...
function lowForecastBatterySaveApplies(state, settings, tariff, forecastContext = null) {
  ...
  // v0.4.0: a proven high-SoC solar day is latched as a sunny day by the app.
  // This override is deliberately valid only for today's solar-day strategy;
  // night planning for tomorrow keeps using tomorrow's own forecast.
  if (context.day === 'today' && Boolean(state.lowForecastSunnyOverrideActive)) return false;
  return Math.max(0, Number(context.forecastKwh)) < thresholdKwh;
}

function normalChargeTarget(settings, targetSoc) {
  return Math.min(targetSoc, clamp(asNumber(settings.dynamicNormalChargeMaxSoc, 50), 0, 100));
}
...
function determineBaseMode(state, settings, tariff, avgSoc, targetSoc, forecastContext = null) {
  ...
  if (isDynamicContract(settings) && tariff.className === 'normal'
    && settings.dynamicNormalChargeEnabled && tariff.selectedChargeMinutes > 0
    && avgSoc + 0.25 < normalChargeTarget(settings, targetSoc)) return 'charge';
...
function computeChargePlanW(state, settings, tariff, avgSoc, targetSoc) {
  if (isDynamicContract(settings) && tariff.className === 'normal') targetSoc = normalChargeTarget(settings, targetSoc);
  ...
  const perBatteryMax = aggregateBatteryPowerLimits(settings, validBatterySocIndices(state, settings)).maxCharge;
...
function buildSocPlan(inputState = {}, rawSettings = {}, now = new Date()) {
  ...
  const state = {
    ...
    lowForecastSunnyOverrideActive: Boolean(inputState.lowForecastSunnyOverrideActive),
    peakGuardExtraLoadW: Math.max(0, asNumber(inputState.peakGuardExtraLoadW, 0)),
  };
  ...
  const batteryCount = normalizedBatteryCount(settings);
  ...
  const phaseBounds = resolvePlanningPhaseBounds(state, settings, now);
  const dynamicScopeEndAt = phaseBounds.targetAt || (now.getTime() + (horizonMinutes * 60000));
  const planningScope = {
    planningReferenceAt: now.getTime(),
    planningPhase: planningCycle?.planningPhase || phaseBounds.phase,
    planningTargetDateKey: planningCycle?.targetDateKey || phaseBounds.targetDateKey,
    planningScopeStartAt: planningCycle?.startAt || phaseBounds.startAt,
    planningChargeDeadlineAt: planningCycle?.chargeDeadlineAt || dynamicScopeEndAt,
    planningPeakStartAt: planningCycle?.peakStartAt || dynamicScopeEndAt,
    planningPeakEndAt: planningCycle?.peakEndAt || dynamicScopeEndAt,
    planningPeakLabel: planningCycle?.peakLabel || phaseBounds.targetLabel,
    planningScopeEndAt: planningCycle?.endAt || dynamicScopeEndAt,
  };
  const scopeLabel = phaseBounds.nightPlanning
    ? 'Nachtplanning tot ingesteld ochtenddoel'
    : 'Dagplanning tot ingesteld dagdoel';
  const rows = [];
  ...
  if (avgSoc === null) {
    return {
      ...
      scopeLabel,
      message: batteryCount === 0 ? 'Geen thuisbatterij geconfigureerd.' : 'Wachten op minstens één geldige batterij-SoC.',
    };
  }
  ...
  if (dynamic) {
    ...
    const expensiveCount = Math.max(1, Math.round((clamp(settings.expensiveHours, 0.25, 12) * 60) / intervalMinutes));
    const expensiveMinutes = new Set(byPrice.slice(-expensiveCount).map(slot => slot.minute));
    let active = null;
    for (const slot of slots) {
      const normal = !cheapMinutes.has(slot.minute) && !expensiveMinutes.has(slot.minute);
      if (!cheapMinutes.has(slot.minute) && !(settings.dynamicNormalChargeEnabled && normal)) continue;
      const tariffLabel = normal ? 'Normaal' : 'Goedkoop';
      ...
      const endAt = Math.min(now.getTime() + (endDelta * 60000), Number(planningScope.planningChargeDeadlineAt));
      if (endAt <= startAt) continue;
      const label = `Goedkoop €${slot.price.toFixed(3)}/kWh`;
      if (active && active.tariffLabel === tariffLabel && startAt <= active.endAt + 1000) {
        active.endAt = Math.max(active.endAt, endAt);
        active.prices.push(slot.price);
      } else {
        if (active) cheapBlocks.push(active);
        active = { startAt, endAt, tariffLabel, price: slot.price, prices: [slot.price], normal };
      }
    }
    if (active) cheapBlocks.push(active);
  ...
  const planBlocks = cheapBlocks.map(block => {
  ...
  for (const block of planBlocks) {
    ...
    const blockTargetSoc = block.normal ? normalChargeTarget(settings, targetSoc) : targetSoc;
    const alreadyPlannedKwh = (energyNeedKwh || 0) - remainingKwh;
    const blockNeedKwh = Math.min(remainingKwh, Math.max(0, capacityKwh * (blockTargetSoc - avgSoc) / 100 - alreadyPlannedKwh));
    let plannedChargeW = 0;
    let chargeKwh = 0;
    let plannedEndAt = null;
    if (targetReady && blockNeedKwh > 0.001 && durationHours > 0 && maxChargeW > 0) {
      if (dynamic) {
        const planUntil = Number(block.availableStartAt) + (12 * 60 * 60 * 1000);
        const availableHours = planBlocks.reduce((sum, candidate) => {
          if (Boolean(candidate.normal) !== Boolean(block.normal)) return sum;
        ...
        plannedChargeW = availableHours > 0
          ? clamp((blockNeedKwh * 1000) / availableHours, 0, maxChargeW)
          : 0;
      } else {
        plannedChargeW = cycleChargeW;
      }
      chargeKwh = Math.min(blockNeedKwh, (plannedChargeW / 1000) * durationHours);
    ...
    rows.push({
      ...
      socTarget: Math.round(blockTargetSoc * 10) / 10,
    });
  }
  ...
  else if (!rows.length) message = 'Geen toegestaan laadvenster gevonden vóór het doelmoment.';
  else if (!hasRemainingChargeWindow) message = 'Alle laadvensters vóór het doelmoment zijn al voorbij; er is geen resterende laadtijd.';
...
function applyPeakGuard(desiredW, state, settings, lastTotalCommandW) {
  ...
  const grid = asNumber(state.gridPowerW, 0) + Math.max(0, asNumber(state.peakGuardExtraLoadW, 0));
...
function distributeCommand(totalCommandW, state, settings, options = {}) {
  ...
  const avg = computeAverageSoc(state, settings);
  ...
  const weights = soc.map((value, index) => {
    // Missing SoC is excluded completely. Min/max SoC and the optional
    // battery-save floor are enforced per battery, never via the pack average.
    if (value === null) return 0;
    if (totalCommandW > 0 && value <= dischargeFloorSoc) return 0;
    if (totalCommandW < 0 && value >= maxSoc) return 0;
    const capacityWeight = settings.individualBatteryPowerLimitsEnabled ? batteryCapacityKwh(settings, index) : 1;
    if (!settings.balanceEnabled || avg === null) return capacityWeight;
    const delta = value - avg;
    if (Math.abs(delta) <= deadband) return capacityWeight;
    if (totalCommandW < 0) return capacityWeight * (delta < 0 ? 1 + strength : 1 - strength);
    if (totalCommandW > 0) return capacityWeight * (delta > 0 ? 1 + strength : 1 - strength);
  ...
  // Keep the legacy allocator byte-for-byte in behaviour unless the user
  // explicitly enables heterogeneous battery limits.
  if (!Boolean(settings.individualBatteryPowerLimitsEnabled)) {
    ...
    for (let pass = 0; pass < 4; pass += 1) {
      ...
      for (const item of eligible) commands[item.index] = clamp(commands[item.index] + share, -maxCharge, maxDischarge);
    }
    return commands.map(value => Math.round(value));
  }

  const charging = totalCommandW < 0;
  const requested = Math.abs(totalCommandW);
  const limits = Array.from({ length: count }, (_, index) => batteryPowerLimits(settings, index));
  const mins = limits.map(item => charging ? item.minCharge : item.minDischarge);
  const maxes = limits.map(item => charging ? item.maxCharge : item.maxDischarge);
  const allocation = weights.map((weight, index) => clamp(requested * (weight / sumWeights), 0, maxes[index]));

  const redistribute = () => {
    for (let pass = 0; pass < count * 3; pass += 1) {
      const actual = allocation.reduce((a, b) => a + b, 0);
      let residual = requested - actual;
      if (residual < 1) break;
      let eligible = allocation
        .map((value, index) => ({ value, index }))
        .filter(item => weights[item.index] > 0 && item.value > 0 && item.value < maxes[item.index] - 0.5);
      if (!eligible.length) {
        const candidate = allocation
          .map((value, index) => ({ value, index, weight: weights[index], min: mins[index], max: maxes[index] }))
          .filter(item => item.weight > 0 && item.value <= 0 && item.max > 0 && residual + 0.001 >= item.min)
          .sort((a, b) => b.weight - a.weight || a.min - b.min)[0];
        if (!candidate) break;
        const startValue = candidate.min > 0 ? candidate.min : Math.min(residual, candidate.max);
        if (startValue <= 0 || startValue > residual + 0.001) break;
        allocation[candidate.index] = Math.min(candidate.max, startValue);
        continue;
      }
      const eligibleWeight = eligible.reduce((sum, item) => sum + Math.max(0.001, weights[item.index]), 0);
      let changed = false;
      for (const item of eligible) {
        const share = residual * (Math.max(0.001, weights[item.index]) / eligibleWeight);
        const next = Math.min(maxes[item.index], allocation[item.index] + share);
        if (next > allocation[item.index] + 0.001) changed = true;
        allocation[item.index] = next;
      }
      if (!changed) break;
    }
  };
  ...
  redistribute();
  // Never round a small request up to a battery's hardware minimum. A command
  // below that minimum becomes 0 W; any remaining demand is then offered to
  // other batteries that can satisfy it safely.
  for (let index = 0; index < allocation.length; index += 1) {
    if (allocation[index] > 0 && allocation[index] + 0.001 < mins[index]) allocation[index] = 0;
  }
  redistribute();

  return allocation.map(value => Math.round(charging ? -value : value) || 0);
}
...
function prepareControlContext(inputState = {}, inputSettings = {}, nowInput = new Date()) {
  ...
  const state = {
    ...
    plannedBatteryGridLimitW: optionalNumber(inputState.plannedBatteryGridLimitW),
...
function evaluate(inputState = {}, inputSettings = {}, nowInput = new Date(), preparedContext = null) {
  ...
  const count = normalizedBatteryCount(settings);
  const configuredBatteryLimits = aggregateBatteryPowerLimits(settings);
  const perBatteryTotalCharge = configuredBatteryLimits.maxCharge;
  const perBatteryTotalDischarge = configuredBatteryLimits.maxDischarge;
  ...
  if (baseMode === 'solar_capture') {
    ...
    // The plan is a charging floor, not a ceiling for live PV capture. Keep
    // the P1 feedback path active at zero as well: resetting to the plan when
    // export stops would undo the previous correction and create export again.
    // A cloud or extra household load reduces capture through the same meter
    // feedback, down to the planned charge power (subject to Peak Guard).
    const meterChargeTargetW = last + (gridErrorW * responseGain);
    // Reconstruct the net surplus from P1 and the last applied battery command.
    // A separate, delayed inverter reading must not cap real measured export.
    const pvSurplusW = Math.max(0, -state.gridPowerW - last);
    const allowedPlanW = state.plannedBatteryGridLimitW === null
      ? plannedChargeW
      : Math.min(plannedChargeW, Math.max(0, state.plannedBatteryGridLimitW) + pvSurplusW);
    desired = Math.min(-allowedPlanW, meterChargeTargetW);
    gridChargeAssistW = Math.max(0, -desired - pvSurplusW);
  ...
  const manualOverrideActive = String(settings.forcedMode || 'auto') !== 'auto';
  const peakReserveProtected = !manualOverrideActive
    && Boolean(targetDetails.peakReserve.active)
  ...
  const peak = count > 0
    ? applyPeakGuard(ordinaryTotalW, state, settings, last)
    : { commandW: 0, active: false, predictedGridW: null };
  ...
  commands = commands.map((value, index) => {
    ...
    const limits = batteryPowerLimits(settings, index);
    if (rounded < 0) {
      const quantizedMaxCharge = Math.floor(limits.maxCharge / commandStepW) * commandStepW;
      const quantizedMinCharge = Math.ceil(limits.minCharge / commandStepW) * commandStepW;
      const magnitude = Math.min(Math.abs(rounded), quantizedMaxCharge);
      if (Boolean(settings.individualBatteryPowerLimitsEnabled) && magnitude > 0 && magnitude < quantizedMinCharge) return 0;
      return -magnitude;
    }
    const quantizedMaxDischarge = Math.floor(limits.maxDischarge / commandStepW) * commandStepW;
    const quantizedMinDischarge = Math.ceil(limits.minDischarge / commandStepW) * commandStepW;
    const magnitude = Math.min(rounded, quantizedMaxDischarge);
    if (Boolean(settings.individualBatteryPowerLimitsEnabled) && magnitude > 0 && magnitude < quantizedMinDischarge) return 0;
    return magnitude;
  });
  ...
  let pvChargeW = 0;
  if (finalChargeW > 0) {
    // Classify the physical energy source from raw P1, not from the strategy
    // name or the EV-adjusted control target. Residual charging while a prior
    // command is being reduced can still draw from the grid in solar_capture.
    const availableSolarW = Math.max(0, -state.gridPowerW - last);
    pvChargeW = Math.min(finalChargeW, availableSolarW);
    gridChargeW = Math.max(0, finalChargeW - pvChargeW);
  }
  ...
  let batteryPauseReason = '';
  let batteryPauseCode = '';
  if (action === 'idle' && finalDischargeW <= actionThresholdW && desired > actionThresholdW) {
    const validSoc = validBatterySocValues(state, settings);
    const allAtFloor = validSoc.length > 0 && validSoc.every(value => value <= ordinaryDischargeFloor + 0.05);
    if (allAtFloor) {
      if (peakReserveProtected && ordinaryDischargeFloor > safetySoc + 0.05) {
        batteryPauseCode = 'planning_floor';
        batteryPauseReason = `Ontladen gepauzeerd · beschermd planningsdoel ${ordinaryDischargeFloor.toFixed(1)}% bereikt`;
      } else if (ordinaryDischargeFloor > minSoc + 0.05) {
        batteryPauseCode = 'discharge_floor';
        batteryPauseReason = `Ontladen gepauzeerd · minimum ontlaad-SoC ${ordinaryDischargeFloor.toFixed(1)}% bereikt`;
      } else {
        batteryPauseCode = 'minimum_soc';
        batteryPauseReason = `Ontladen gepauzeerd · minimum SoC ${ordinaryDischargeFloor.toFixed(1)}% bereikt`;
      }
    }
  }
  ...
  return {
    ...
    lowForecastSunnyOverrideActive: Boolean(state.lowForecastSunnyOverrideActive) && strategyForecastContext.day === 'today',
    ...
    batteryPauseReason,
    batteryPauseCode,
    batteryDischargeFloorSoc: Math.round(ordinaryDischargeFloor * 10) / 10,
    peakGuardExtraLoadW: Math.round(state.peakGuardExtraLoadW),
  };
}
