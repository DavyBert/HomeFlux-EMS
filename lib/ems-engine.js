...
const EV_INSTANCE_DEFAULT_KEYS = [
  ...
  'PvGridTopUpMode','PvStartSurplusW','PvStopSurplusW','PvStopDelaySeconds','Phases',
  ...
  const redistribute = () => {
    for (let pass = 0; pass < count * 3; pass += 1) {
      ...
      for (const item of eligible) {
        residual = requested - allocation.reduce((a, b) => a + b, 0);
        if (residual < 1) break;
        const share = residual * (Math.max(0.001, weights[item.index]) / eligibleWeight);
        const next = Math.min(maxes[item.index], allocation[item.index] + Math.max(1, share));
