...
function compilePriceFormula(expression) {
  ...
  if (!raw || raw.length > 2048) throw new Error('Unsupported Homey price formula');
  // Labels and supported price tokens all refer to the same EUR/kWh input.
  // Replace only known identifiers, never arbitrary tags or executable text.
  const priceName = '(?:p|x|price|value|marketPrice|spotPrice|electricityPrice|basePrice|base_price|basisprijs|basis_prijs|base\\s+price|basis\\s+prijs)';
  const wrapped = new RegExp('\\[\\[\\s*(' + priceName + ')\\s*\\]\\]|\\{\\{\\s*(' + priceName + ')\\s*\\}\\}|\\{\\s*(' + priceName + ')\\s*\\}|\\[\\s*(' + priceName + ')\\s*\\]', 'gi');
  let text = raw.replace(wrapped, 'price')
    .replace(/\b(?:base\s+price|basis\s+prijs)\b/gi, 'price');
  // Homey wraps the complete arithmetic expression in {{ ... }}.
  // Strip only one complete wrapper; stray/nested braces still fail validation.
  const envelope = /^\{\{([^{}]*)\}\}$/.exec(text);
  if (envelope) text = envelope[1].trim();
  ...
  function atom() {
    ...
    } else if (/^(p|x|price|value|marketPrice|spotPrice|electricityPrice|basePrice|base_price|basisprijs|basis_prijs)$/i.test(token || '')) fn = p => p;
