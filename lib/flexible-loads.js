...
function evModeEstimatedCurrents(settings = {}) {
  ...
  const standardA = clamp(Math.round(Number(settings.evModeStandardCurrentA) || Number(settings.evStandardCurrentA) || 16), 1, 64);
...
function getEvPvTariffPolicy(settings = {}, tariff = null) {
  ...
  const generalStopW = Math.max(0, Number(settings.evPvStopSurplusW) || 0);
  ...
  return {
    ...
    minSurplusW: Math.max(generalStartW, Math.max(0, Number(rate.evPvMinSurplusW) || 0)),
    stopSurplusW: generalStopW,
...
function calculateEvDecision(input = {}) {
  ...
  const socPlanActive = Boolean(settings.evSocPlanActive);
  ...
  const socFallbackActive = mode !== 'emergency' && !energyPlanActive && (!socEnabled || !socFresh);
