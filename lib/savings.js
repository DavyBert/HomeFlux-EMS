...
function emptyDay(date = '') {
  return {
    ...
    exportedEnergyKwh: 0,
    exportedEnergyKnown: false,
    directGridKwh: 0,
    directGridCost: 0,
    directPvKwh: 0,
    directPvValue: 0,
    directPvExportKwh: 0,
    directPvExportValue: 0,
    pvBatteryKwh: 0,
    pvBatteryValue: 0,
    pvBatteryHomeKwh: 0,
    pvBatteryHomeValue: 0,
    pvBatteryExportKwh: 0,
    pvBatteryExportValue: 0,
    shiftKwh: 0,
    shiftValue: 0,
    shiftHomeKwh: 0,
    shiftHomeValue: 0,
...
function normalizeDay(raw, date = '') {
  ...
  const signedKeys = new Set(['directGridCost', 'gridChargeCost']);
  for (const key of Object.keys(out)) {
    if (key === 'date' || key === 'chargeCostsByTariff' || key === 'importedEnergyKnown' || key === 'exportedEnergyKnown') continue;
    out[key] = Math.max(key.endsWith('Value') || signedKeys.has(key) ? -Infinity : 0, n(raw?.[key], 0));
  }
  out.importedEnergyKnown = Boolean(raw?.importedEnergyKnown);
  out.exportedEnergyKnown = Boolean(raw?.exportedEnergyKnown);

  // Backward compatibility for pre-0.7.3 savings records: pvBatteryValue used
  // to contain both home-use value and feed-in value. Preserve the difference
  // as export value so totals do not drop when the new explicit export bucket
  // is introduced.
  if (raw && raw.pvBatteryExportValue === undefined) {
    const legacyTotal = n(raw.pvBatteryValue, 0);
    const legacyHome = n(raw.pvBatteryHomeValue, 0);
    out.pvBatteryExportValue = legacyTotal - legacyHome;
  }
  out.pvBatteryValue = out.pvBatteryHomeValue + out.pvBatteryExportValue;
...
function pvExportValue(day) {
  return n(day?.directPvExportValue) + n(day?.pvBatteryExportValue);
}

function pvExportKwh(day) {
  return Math.max(0, n(day?.directPvExportKwh)) + Math.max(0, n(day?.pvBatteryExportKwh));
}

function totalSavings(day) {
  return n(day?.directPvValue)
    + n(day?.pvBatteryHomeValue)
    + pvExportValue(day)
    + n(day?.shiftValue);
}

function avoidedEnergyValue(day) {
  // Signed purchase prices matter here too: consuming/storing PV while the grid
  // price is negative can have a negative avoided-energy value.
  return n(day?.directPvValue) + n(day?.pvBatteryHomeValue);
}
...
function rawExportedKwh(day) {
  return Math.max(0, n(day?.directPvExportKwh))
    + Math.max(0, n(day?.pvBatteryExportKwh))
    + Math.max(0, n(day?.shiftExportKwh))
    + Math.max(0, n(day?.unknownBatteryExportKwh));
}
...
function calibrateExportedEnergy(day) {
  const out = normalizeDay(day, day?.date || '');
  const targetKwh = Math.max(0, n(out.exportedEnergyKwh));
  const measuredKwh = rawExportedKwh(out);
  if (!out.exportedEnergyKnown || !(measuredKwh > 0)) return out;

  const factor = targetKwh / measuredKwh;
  out.directPvExportKwh *= factor;
  out.directPvExportValue *= factor;
  out.pvBatteryExportKwh *= factor;
  out.pvBatteryExportValue *= factor;
  out.unknownBatteryExportKwh *= factor;

  const splitShiftKnown = Math.max(0, n(out.shiftHomeKwh)) + Math.max(0, n(out.shiftExportKwh)) > 0
    || Math.abs(n(out.shiftHomeValue)) + Math.abs(n(out.shiftExportValue)) > 0;
  out.shiftExportKwh *= factor;
  out.shiftExportValue *= factor;
  if (splitShiftKnown) {
    out.shiftKwh = Math.max(0, n(out.shiftHomeKwh)) + Math.max(0, n(out.shiftExportKwh));
    out.shiftValue = n(out.shiftHomeValue) + n(out.shiftExportValue);
  }
  out.pvBatteryKwh = Math.max(0, n(out.pvBatteryHomeKwh)) + Math.max(0, n(out.pvBatteryExportKwh));
  out.pvBatteryValue = n(out.pvBatteryHomeValue) + n(out.pvBatteryExportValue);
  return out;
}

function calibrateEnergy(day) {
  return calibrateExportedEnergy(calibrateImportedEnergy(day));
}
...
function integrateInterval({ day, inventory, seconds, gridW, pvW, batteryW, importPrice, feedInPrice = 0, tariff, capacityKwh }) {
  ...
  const exportPrice = n(feedInPrice);
  ...
  const directPvExportW = Math.min(
    Math.max(0, remainingPvW - pvToBatteryW),
    Math.max(0, -grid - batteryToGridW),
  );

  const directGridKwh = (gridToLoadW / 1000) * dtHours;
  day.directGridKwh += directGridKwh;
  day.directGridCost += directGridKwh * n(importPrice);

  const directPvKwh = (pvToLoadW / 1000) * dtHours;
  day.directPvKwh += directPvKwh;
  day.directPvValue += directPvKwh * n(importPrice);

  const directPvExportKwh = (directPvExportW / 1000) * dtHours;
  day.directPvExportKwh += directPvExportKwh;
  day.directPvExportValue += directPvExportKwh * exportPrice;
  ...
  if (gridChargeKwh > 0) {
    const chargeCost = gridChargeKwh * n(importPrice);
  ...
  const pvBatteryExportKwh = consumed.pvKwh * exportShare;
  const pvHomeValue = pvHomeKwh * n(importPrice);
  const pvBatteryExportValue = pvBatteryExportKwh * exportPrice;
  day.pvBatteryHomeKwh += pvHomeKwh;
  day.pvBatteryHomeValue += pvHomeValue;
  day.pvBatteryExportKwh += pvBatteryExportKwh;
  day.pvBatteryExportValue += pvBatteryExportValue;
  day.pvBatteryKwh += consumed.pvKwh;
  day.pvBatteryValue += pvHomeValue + pvBatteryExportValue;
  day.unknownBatteryExportKwh += consumed.unknownKwh * exportShare;
  ...
  for (const item of Object.values(consumed.grid)) {
    ...
    const homeValue = gridHomeKwh * (n(importPrice) - sourceCostPerKwh);
    const exportValue = gridExportKwh * (exportPrice - sourceCostPerKwh);
    day.shiftHomeKwh += gridHomeKwh;
    day.shiftHomeValue += homeValue;
    day.shiftExportKwh += gridExportKwh;
    day.shiftExportValue += exportValue;
    day.shiftKwh += item.kwh;
    day.shiftValue += homeValue + exportValue;
  }
}
...
function addDays(target, source) {
  ...
  target.directPvExportKwh += n(source?.directPvExportKwh);
  target.directPvExportValue += n(source?.directPvExportValue);
  ...
  target.pvBatteryExportKwh += n(source?.pvBatteryExportKwh);
  target.pvBatteryExportValue += n(source?.pvBatteryExportValue);
  target.shiftKwh += n(source?.shiftKwh);
  target.shiftValue += n(source?.shiftValue);
  target.shiftHomeKwh += n(source?.shiftHomeKwh);
  target.shiftHomeValue += n(source?.shiftHomeValue);
  target.shiftExportKwh += n(source?.shiftExportKwh);
  target.shiftExportValue += n(source?.shiftExportValue);
  target.unknownBatteryExportKwh += n(source?.unknownBatteryExportKwh);
